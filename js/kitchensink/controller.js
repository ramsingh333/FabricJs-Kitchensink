...
function addAccessors($scope) {
    ...
    $scope.addImage1 = function () {
        addImage('audio_speakers.png', 0.1, 0.25);
    };

    $scope.addImage2 = function () {
        addImage('Flat-Screen-TV.jpg', 0.1, 0.50);
    };

    $scope.addImage3 = function () {
        addImage('Soundbar.png', 0.1, 0.25);
    };
    $scope.addImage4 = function () {
        addImage('camera.png', 0.1, 0.25);
    };

    $scope.addImage5 = function () {
        addImage('Wireconceal.png', 0.1, 0.25);
    };
    ...
    $scope.rasterize = function (multiplier) {
        ...
        if (!fabric.Canvas.supports('toDataURL')) {
            ...
            html2canvas([document.getElementById('dvexport')], {
                onrendered: function (canvas) {
                    document.body.appendChild(canvas);
                    var data = canvas.toDataURL('image/png');
                    // AJAX call to send `data` to a PHP file that creates an image from the dataURI string and saves it to a directory on the server
                    var a = $("<a>").attr("href", data).attr("download", "img.png").appendTo("body");
